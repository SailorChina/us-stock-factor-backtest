...
class Strategy(StrategyBase):
    ...
    def _get_price(self, context, code):
        """获取最新价格"""
        try:
            # 富途平台内置函数：get_cur_kline(code, num, ktype)
            data = get_cur_kline(code, 1, '1d')
            if data and len(data) > 0:
                return data[0].get('close', None) if isinstance(data, list) else data.get('close', None)
    ...
    def _get_klines(self, context, code, count):
        ...
        try:
            # 富途平台内置函数：get_cur_kline(code, num, ktype)
            klines = get_cur_kline(code, count, '1d')
            return klines
        except Exception as e:
    ...
    def _buy(self, context, code, qty):
        ...
        try:
            ...
            # 平台下单函数（示例）
            # order_target_value(code, qty * self._get_price(context, code))
            self.trade_count += 1
        except Exception as e:
