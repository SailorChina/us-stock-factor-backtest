...
if "US.VOO" in fcodes:
    p0 = close["US.VOO"].iloc[start]; sh = int(3000//p0)
    fee0 = max(sh*COMM_PER_SHARE, COMM_MIN)
    cash_voo = 3000 - sh*p0 - fee0
    eq_voo = pd.Series(cash_voo + sh*close["US.VOO"].values, index=dates)
...
plt.tight_layout(); plt.savefig("backtest_v2_equity.png", dpi=130); plt.close()
...
json.dump(out, open("backtest_v2_results.json","w"), ensure_ascii=False, indent=2)
